...
- **External quota/usage data: local files first, endpoint with cache+backoff second, always degrade gracefully.** Read from disk where the tool persists it (Codex), else hit the tool's own endpoint (Claude `/api/oauth/usage`) with caching + 429 backoff. Treat any source as optional → `available:false`/N-A on failure, never block the UI. **Never log or persist OAuth tokens** (memory-only). → ADR-0003
- **Credential/secret-bearing I/O lives in the backend node service, never the frontend.** Frontend gets only normalized, non-secret data over JSON-RPC. → ADR-0003
